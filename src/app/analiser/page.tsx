...
export default function Analizer() {
    ...
    if (progess < 100) {
        return (
            <main className="flex min-h-screen items-center justify-center min-w-screen">
    ...
    return (
        <main className="flex min-h-screen items-center justify-center gap-2">
            ...
            <main className="flex flex-col items-center justify-start gap-2 w-1/4 xl:h-[48rem] lg:h-[32rem] md:h-[24rem] sm:h-[16rem] border rounded-md overflow-hidden">
                ...
                <div className="flex flex-wrap w-full h-full max-h-full items-start overflow-y-auto">
                    {history.current.map((move, index) => {
                        return <div key={index} className={"flex justify-center items-center w-1/2 border" + (index % 2 == 1 ? " bg-gray-200 border-gray-500" : "") + (index == moving-1 ? ' bg-green-100' : '')}>
                            {
